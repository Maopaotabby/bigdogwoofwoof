(function attachDuelCardTemplate(global) {
  ...
  function getDuelCharacterCombatStats(characterOrActor) {
    ...
    var stats = {
      cePool: normalizeDuelRank(source.cePool || source.maxCeRank || source.cursedEnergy || baseRank, baseRank),
      ceOutput: normalizeDuelRank(source.ceOutput || source.cursedEnergyOutput || source.output || baseRank, baseRank),
      ceControl: normalizeDuelRank(source.ceControl || source.control || baseRank, baseRank),
      ceEfficiency: normalizeDuelRank(source.ceEfficiency || source.efficiency || baseRank, baseRank),
      techniquePower: normalizeDuelRank(source.techniquePower || source.technique || baseRank, baseRank),
      physicalPower: normalizeDuelRank(source.physicalPower || source.body || source.physical || baseRank, baseRank),
      speed: normalizeDuelRank(source.speed || source.mobility || baseRank, baseRank),
  ...
  function calculateDuelDamageFromCard(actionOrCandidate, characterOrActor, options) {
    ...
    if (base.scalingProfile === "technique") value *= m.ceOutput * Math.pow(m.ceControl, 0.5) * Math.pow(m.techniquePower, 0.7) * contextMultiplier;
    else if (base.scalingProfile === "ce_burst") value *= Math.pow(m.ceOutput, 1.1) * Math.pow(m.ceControl, 0.4) * riskMultiplier;
  ...
  function calculateDuelDomainPressureFromCard(actionOrCandidate, characterOrActor) {
    ...
    var stats = getDuelCharacterCombatStats(characterOrActor || {});
    var m = stats.multipliers;
    return roundPreviewNumber(base.baseDomainPressure * m.domainSkill * Math.pow(m.ceOutput, 0.5) * Math.pow(m.ceControl, 0.5), 1);
  }
