...
function isTaskApplicable(task) {
  ...
  if (task.nodeId === "innateTechnique") return hasTechniqueNow() && state.flags.familyInheritedTechnique !== true;
  if (task.nodeId === "hasTechnique" && state.flags.familyInheritedTechnique === true) return false;
  ...
  if (task.nodeId === "domainType" || task.nodeId === "domainEffectCount" || task.nodeId === "domainEffects") {
...
function evaluateCondition(condition, task) {
  ...
  if (condition === "location includes 西姆利亚星") return getAnswerText("location").includes("西姆利亚星");
...
function updateFlags(task, text, result, optionEffectTrace = null) {
  ...
  switch (task.nodeId) {
    ...
    case "location":
      ...
      if (text.includes("西姆利亚星")) {
        state.flags.isAlien = true;
        state.flags.identityOverride = "西姆利亚星人";
      }
      break;
    ...
    case "customTechniqueTypes":
      ...
      break;
    case "specialTalent":
      state.flags.specialTalent = text;
      break;
...
function getWeightedOptions(wheel, task, context = {}) {
  return callSiteModuleImplementation("JJKLifeWheelRandom", "buildWeightedOptions", [wheel, task, {
    context,
    normalizeOptionText,
    getAdjustedWeight,
...
function computeAbilityBuildBonus(options = {}) {
  ...
  if (specialTalent.includes("超级受肉体")) bonus += 0.46;
  if (specialTalent.includes("半人半咒")) bonus += 0.38;
  if (specialTalent.includes("星浆体")) bonus += 0.18;
...
function computeSelectedDomainEffectSynergy(profile) {
  const effects = state.flags.domainEffects || [];
...
function adjustDomainEffectCountWeight(text, baseWeight) {
  ...
  const quality = computeDomainQualityScore();
  let multiplier = 1;
  if (count <= 1) multiplier *= quality >= 6 ? 0.72 : 1.12;
  if (count === 2) multiplier *= quality >= 6 ? 0.95 : 1;
  if (count >= 3) multiplier *= 1 + clamp((quality - 5.4) * 0.12, -0.22, 0.55);
  if (count >= 5) multiplier *= quality >= 7 ? 1.18 : 0.78;
  return Math.max(0.05, baseWeight * multiplier);
}
